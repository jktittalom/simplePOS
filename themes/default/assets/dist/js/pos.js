function add_invoice_item(t) {
    if ((1 == count && (spositems = {}), null != t)) {
        ...
        return spositems[e] ? (spositems[e].row.qty = parseFloat(spositems[e].row.qty) + parseFloat(t.row.qty || 1)) : (spositems[e] = t), store("spositems", JSON.stringify(spositems)), loadItems(), !0;
    }
}
function loadItems() {
    if ((1 == count && (spositems = {}), get("spositems"))) {
        (total = 0),(final_total = 0), (count = 1), (an = 1), (product_tax = 0), (invoice_tax = 0), (product_discount = 0), (order_discount = 0), (total_discount = 0), $("#posTable tbody").empty();
        var t = new Date().getTime() / 1e3;
        if (1 != Settings.remote_printing) {
            var e =
                (n = "C: " + $("#select2-spos_customer-container").text() + "\n") +
                (s = "R: " + $("#hold_ref").val() + "\n") +
                (r = "U: " + username + "\n") +
            ...
            $("#order_span").prepend(a + o + "<h4>" + lang.order + "</h4></span>" + n + s + r + i), $("#bill_span").prepend(a + o + "<h4>" + lang.bill + "</h4></span>" + n + s + r + i), $("#order-table").empty(), $("#bill-table").empty();
        }
               ...
               /* console.log('Jiten: product_tax: '+product_tax);*/
                ...
                (tr_html =
                    ...
                    (tr_html +=
                        ...
                        h +
                        '">' +
                        formatMoney(parseFloat(p) + parseFloat(v)) +
                        "</span></td>"),
                    (tr_html +=
                        '<td><input name="item_was_ordered[]" type="hidden" class="riwo" value="0"><input class="form-control input-qty kb-pad text-center rquantity" name="quantity[]" type="text" value="' +
                        formatDecimal(r) +
                        '" data-id="' +
                        h +
                        '" data-item="' +
                    ...
                    "standard" == o && i < r
                        ...
                        : "combo" == o &&
                          (!1 === s
                              ? $("#" + h).addClass("danger")
                              : $.each(s, function () {
                                    parseFloat(this.quantity) < parseFloat(this.qty) * r &&
                                        ($("#" + h).addClass("danger"),
                                        $("#" + h)
                                            .find(".edit")
                                            .removeClass("bg-purple")
                ...
                if (1 != Settings.remote_printing) {
                    ...
                    var M = '<tr class="row_' + e + '" data-item-id="' + e + '"><td colspan="2" style="border-bottom:0!important;">#' + (an - 1) + " " + (0 == e ? t.row.name : d + " (" + c + ")");
                    for (k = 0, w = x.length; k < w; k++) M += x[k] ? "<br> <b>*</b> <small>" + x[k] + "</small>" : "";
                    (M += "</td></tr>"),
                        (M +=
                            '<tr class="bb row_' +
                            e +
                            ...
                            formatMoney((parseFloat(p) + parseFloat(v)) * parseFloat(r)) +
                            "</td></tr>");
                    var F = '<tr class="bb row_' + e + '" data-item-id="' + e + '"><td>#' + (an - 1) + " " + d + " (" + c + ")";
                    for (k = 0, w = x.length; k < w; k++) F += x[k] ? "<br> <b>*</b> <small>" + x[k] + "</small>" : "";
                    (F += "</td><td>[ " + parseFloat(r) + " ]</td></tr>"), $("#order-table").append(F), $("#bill-table").append(M);
                }
            });
        var l = get("spos_discount") ? get("spos_discount") : $("#discount_val").val() ? $("#discount_val").val() : "0";
        if (((order_discount = parseFloat(l)), -1 !== l.indexOf("%"))) {
            var c = l.split("%");
        ...
        */
        ...
        if (
            ...
            (order_data.items = S), (bill_data.items = D);
            var m = "";
            if (
                ((m += printLine(lang.total + ": " + formatMoney(total)) + "\n"),
            ...
            var g = "";
            if (
                ((g += '<tr class="bb"><td>' + lang.total_items + '</td><td style="text-align:right;">' + (an - 1) + " (" + (parseFloat(count) - 1) + ")</td></tr>"),
                ...
                (g += '<tr class="bb"><td>' + lang.rounding + '</td><td style="text-align:right;">' + formatMoney(_) + "</td></tr>"),
                    (g += "<tr><td>" + lang.total_payable + '</td><td style="text-align:right;">' + formatMoney(round_total) + "</td></tr>");
            }
            (g += '<tr><td colspan="2" style="text-align:center;">' + lang.merchant_copy + "</td></tr>"), $("#bill-total-table").empty(), $("#bill-total-table").append(g);
        }
...
function Popup(t, e) {
    $("#print-title").text(lang.print + " " + lang[e]), $("#print-body").html(t), $("#printModal").show();
}
...
function posScreen() {
    ...
    $("#right-col").height(t - 100), $(".items").height(400 < o ? o : 400), $("#list-table-div").height(n);
}
...
$(document).ready(function () {
    $(document).on("click", ".no-results, #filter-suspended-sales", function (t) {
        ...
        $("#filter-categories").hideseek({ nodata: lang.no_match_found }),
        $(document).on("click", ".suspended_sales .dropdown-menu .header", function (t) {
            t.stopPropagation();
        }),
        $("#filter-suspended-sales").hideseek({ nodata: lang.no_match_found }),
        ...
        $("#posTable").on("click", ".edit", function () {
            var t = $(this).closest("tr"),
                e = (t.attr("id"), t.attr("data-item-id")),
                a = t.attr("data-id"),
                o = spositems[e],
                n = formatDecimal(t.find(".realuprice").val()),
                s = n,
                r = o.row.discount ? o.row.discount : "0";
            if (((item_discount = formatDecimal(parseFloat(r))), -1 !== r.indexOf("%"))) {
                var i = r.split("%");
                isNaN(i[0]) || (item_discount = formatDecimal((s * parseFloat(i[0])) / 100));
            }
            s -= item_discount;
            var l = parseFloat(o.row.tax),
                c = 0,
                d = "";
            null !== l && 0 != l && (d = 0 == parseFloat(o.row.tax_method) ? ((s -= c = formatDecimal((s * parseFloat(l)) / (100 + parseFloat(l)))), lang.inclusive) : ((c = formatDecimal((s * parseFloat(l)) / 100)), lang.exclusive)),
                $("#proModalLabel").html('<a href="' + base_url + "/products/view/" + a + '" data-toggle="ajax">' + o.label + "</a>"),
                $("#net_price").text(formatMoney(s)),
                $("#pro_tax").text(formatMoney(c)),
                $("#pro_tax_method").text("(" + d + ")"),
                $("#row_id").val(row_id),
                $("#item_id").val(e),
                $("#nPrice").val(n),
                $("#nQuantity").val(o.row.qty),
                $("#nDiscount").val(r),
                $("#nComment").val(o.row.comment),
        ...
        $(document).on("change", "#nPrice, #nDiscount", function () {
            ...
            null !== r &&
                0 != r &&
                (0 == parseFloat(o.row.tax_method) ? ((a -= i = formatDecimal((a * parseFloat(r)) / (100 + parseFloat(r)))), (tax = lang.inclusive)) : ((i = formatDecimal((a * parseFloat(r)) / 100)), (tax = lang.exclusive))),
            ...
            1 == protect_delete
                ? bootbox
                      .dialog({
                          ...
                          buttons: {
                              danger: { label: lang.close, className: "btn-default pull-left", callback: function () {} },
                              success: {
                                  label: "<i class='fa fa-tick'></i> " + lang.delete,
                                  className: "btn-warning verify_pin",
                                  callback: function () {
                                      md5($("#pos_pin").val()) == Settings.pin_code
                                          ? (get("spositems") && remove("spositems"),
        ...
        $("#print_order").click(function (t) {
            if ((t.preventDefault(), count <= 1)) bootbox.alert(lang.please_add_product);
                ...
                if (($("#order-data").show(), 1 == Settings.print_img)) {
                    ...
                    html2canvas(e, { scrollY: 0, scale: 1.3 }).then(function (t) {
        ...
        $("#add_item").autocomplete({
            ...
            response: function (t, e) {
                16 <= $(this).val().length && e.content[0] && 0 == e.content[0].id
                    ? (bootbox.alert(lang.no_match_found, function () {
                          $("#add_item").focus();
                      }),
                      $(this).val(""))
                    : 1 == e.content.length && e.content[0] && 0 != e.content[0].id
                    ? ((e.item = e.content[0]), $(this).data("ui-autocomplete")._trigger("select", "autocompleteselect", e), $(this).autocomplete("close"))
                    : 1 == e.content.length &&
                      e.content[0] &&
                      0 == e.content[0].id &&
                      (bootbox.alert(lang.no_match_found, function () {
        ...
        $(document).on("click", "#addGiftCard", function (t) {
            ...
            (s[0] = e),
                ...
                $.ajax({
                    ...
                    success: function (t) {
                        "success" === t.result
                            ? ((spositems[0] = { id: 0, item_id: 0, label: a + " (" + e + ")", row: { id: 0, code: e, name: a, quantity: 1, price: n, real_unit_price: n, tax: 0, qty: 1, type: "manual", discount: "0", comment: "" } }),
                              store("spositems", JSON.stringify(spositems)),
                              loadItems(),
                              $("#gcModal").modal("hide"),
                              $("#gccard_no").val(""),
                              $("#gcvalue").val(""),
                              $("#gcprice").val(""))
                            : ($("#gcerror").text(t.message), $(".gcerror-con").show());
                    },
                });
        });
    $(document).on("click", ".posdel", function () {
        ...
        $("#payment").click(function () {
            if (count <= 1) return bootbox.alert(lang.please_add_product), !1;
            if (
                (sid && ((suspend = $("<span></span>")), suspend.html('<input type="hidden" name="delete_id" value="' + sid + '" />'), suspend.appendTo("#hidesuspend")),
        ...
        $("#amount").change(function (t) {
            ...
            $("#total_paying").text(formatMoney(e)),
                0 != Settings.rounding
                    ? ($("#balance").text(formatMoney(e - round_total)), $("#balance_val").val(formatDecimal(e - round_total)), (total_paid = e), (grand_total = round_total))
                    : ($("#balance").text(formatMoney(e - gtotal)), $("#balance_val").val(formatDecimal(e - gtotal)), (total_paid = e), (grand_total = gtotal));
        }),
        $("#add-customer").click(function () {
            $("#customerModal").modal({ backdrop: "static" });
        }),
        $("#payModal").on("keypress", function (t) {
            console.log(t.target.type),
                13 == t.which &&
        ...
        $("#payModal").on("change", "#paid_by", function () {
            ...
            if (0 != Settings.rounding) var a = formatDecimal(roundNumber(e, parseInt(Settings.rounding)));
            else a = formatDecimal(e);
            $("#rpaidby").val(t),
                "gift_card" == t
                    ? ($(".gc").slideDown(),
                      $(".ngc").slideUp("fast"),
                    ...
                    : ($(".ngc").slideDown(), $(".gc").slideUp("fast"), $("#gc_details").html(""), $("#amount").attr("readonly", !1)),
                "cash" == t || "other" == t
                    ? ($(".pcash").slideDown(),
        ...
        (spos_note = get("spos_note")) && ($("#note").val(spos_note), $("#snote").val(spos_note)),
        ...
        $("#submit-sale").click(function () {
            $("#total_items").val(an - 1), $("#total_quantity").val(quantityFormat(count - 1)), $("#submit").click();
        });
    var t = $("#hold_ref").val();
    $("#hold_ref").change(function () {
        ...
        $("#suspend_sale").click(function () {
            return $("#reference_note").val() && ($("#hold_ref").val($("#reference_note").val()), $("#total_items").val(an - 1), $("#total_quantity").val(quantityFormat(count - 1)), $("#submit").click()), !1;
        }),
        $("#customer-form").on("submit", function (t) {
            return (
                ...
                $.ajax({
                    ...
                    success: function (t) {
                        "success" == t.status
                            ? ($("#spos_customer").append($("<option></option>").attr("value", t.id).text(t.val)), $("#spos_customer").select2("val", t.id), $("#customerModal").modal("hide"))
                            : ($("#c-alert").html(t.msg), $("#c-alert").show());
                    },
                    error: function () {
    ...
    $(document).ready(function (e) {
        window.setTimeout(function () {
            ...
            e("#list-table-div").slimScroll({ start: "bottom" }),
            e("#category-sidebar-menu").slimScroll({ width: "100%" }),
            e(".items").slimScroll({});
    }),
