...
class Pos extends MY_Controller
{
    ...
    public function get_product($code = null)
    {
        ...
        $combo_items = false;
        //if ($product = $this->pos_model->getProductByCode($code)) { // by Jiten 18 Aug 2021
        $store = $this->site->getStoreByID($this->session->userdata('store_id'));
        if ($product = $this->pos_model->getProductByCode_Jiten($code, substr($store->vat, 0, -1))) {
            ...
            $product->unit_price      =$product->product_tax+$product->price;
            ...
            for ($r = 0; $r < $i; $r++) {
                ...
                if (isset($item_id) && isset($real_unit_price) && isset($item_quantity)) {
                   ...
                   // $unit_price = $real_unit_price;
                    ...
                    $unit_price = ($product_details->store_price > 0 ? $product_details->store_price :  $product_details->price);
                    ...
                    /*
                    //comment by Jiten, As the tax is inclusiv and store wise
    ...
    public function suggestions()
    {
        ...
        if (is_array($term)) {
            ...
            $term   = $term['item_code'];
            //$rows   = $this->pos_model->getProductNames($term, null, true,); // By Jiten
            $rows   = $this->pos_model->getProductNames_Jiten($term, null, true, substr($store->vat, 0, -1));
        }
        if (!$rows) {
            $bqty   = null;
            $bprice = null;
            $term   = $this->input->get('term', true);
           // $rows   = $this->pos_model->getProductNames($term); //By Jiten
            $rows   = $this->pos_model->getProductNames_Jiten($term,10, false, substr($store->vat, 0, -1));
        }
        if ($rows) {
            foreach ($rows as $row) {
                ...
                //$row->unit_price      = $this->Settings->default_vat ? ($row->price + (($row->price * $this->Settings->default_vat) / 100)) : $row->price; // By Jiten: 18 Aug 2021
